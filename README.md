...
Inspecting the folder structur will reveal a lot of scaffolding which is already done for us by the simple project leaving us with minor editing.
...
-- add one to it.
-- (Note, this is not the best way of implementing very easy to implement,
-- which is our goal right now)
--
-- Clash provides us with the ability to define arbitrary bounded unsigned
-- integers via `Index n`. Thus what we are describing here is a an integer
...
-- accumulator and possible flipping the state of our `led`.
...
-- it determines that our module does use more than one. In this case the
...
-- right frequency. Then you would need use `Clock MyDomain -> Signal MyDomain
-- Bit` here.
--
...
Depending on the board you use, remember I use the Arty A7-35T, you need to select the right in the next step.
...
Save the file.
Right click `helloworld.xdc` in thhe `Sources` pane and click on `Set as target constraint file`.
...
The bitstream file is located in `$PATHTOVIVADOPROJECT/fpgablinker/fpgablinker.runs/impl_1/helloworld.bit`.
...
Maybe I was able to alleviate some of the introductions, especially in regards to Clash and FPGA programming.
